"""
CLIP-based zero-shot classifier analyzer branch.

Dispatched when `model_type=zero_shot`. Wraps the zero-shot scoring
script under `dreams_laboratory/scripts/` to classify viewport tiles
against user-provided class names; the return payload contains
per-class confidence scores plus the top prediction. Heavy imports
(transformers, torch, PIL) are function-local.
"""
