...
def _analyze_viewport_grounding_dino(image, location, text_prompt, box_threshold, text_threshold, scripts_dir):
    ...
    try:
        ...
        # Initialize detector with CUDA if available, fallback to CPU if CUDA fails
        detector = None
        device = 'cuda' if cuda_available else 'cpu'
        ...
        if cuda_available:
            try:
                ...
                # CUDA ops may not be compiled, fallback to CPU
        ...
        # Initialize with CPU if CUDA failed or not available
        if detector is None:
            try:
